---
show: survivor
number: 12
title: Panama
display_title: "Panama: <em>Exile</em><br/>Island"
premiere_date: 2006-02-02
ep_count: 14
location: Pearl Islands, Panama
host: Jeff Probst
format_changes: [exile-island, hidden-immunity-idol, four-tribe-opening]
eyebrow: "Aired spring 2006 · Filmed in Panama"
lede: "The season that introduces the two mechanics modern Survivor leans on hardest — Exile Island and the hidden immunity idol. Sixteen castaways drop in as four tribes of four, an opening structure the show has never quite tried at this density since."
pull: "Panama is the moment the modern game gets installed. The hidden idol and Exile Island arrive in the same season, and the show never fully looks back."
filming_caption: "Pearl Islands · returning S07/S08 country"
premiere_caption: "CBS · Thursday 8/7c"
episodes_caption: "39 days in country"
format_summary: "Originals · 4 tribes → 2"
format_caption: "first hidden idol, first Exile"
cast_size: 16
cast_size_caption: "4 Casaya, 4 La Mina, 4 Bayoneta, 4 Viveros"
host_caption: "twelfth season at the helm"
watch_list:
  - episode_label: "Ep 1 · four-tribe split"
    body: "The marooning divides the cast four ways and forces a vote before anybody has settled in. The structural pressure on the first episode is heavier than any premiere the show had attempted up to this point."
  - episode_label: "Ep 3 · Exile arrives"
    body: "The Exile Island mechanic enters the show for the first time. Watch the early-season cast figure out what banishment to an isolated beach actually does to a player's social standing."
  - episode_label: "Ep 6 · the idol"
    body: "The hidden immunity idol is introduced and the cast starts to grasp what kind of object it really is. The idol's grammar gets written across the back half of the season."
  - episode_label: "Ep 11 · post-merge"
    body: "The endgame is the first time the show plays a merge with both Exile Island and an idol live in the game. The strategic texture of every season since starts here."
---

The twelfth season returns to the Pearl Islands and installs
the two pieces of grammar the modern game runs on — Exile
Island and the hidden immunity idol. The opening splits the
cast into four small tribes that collapse to two after a vote,
a structural premise the show has never quite repeated at this
scale. Panama is a season-long pilot for what Survivor would
become from this point forward, and the cast plays the
new game with real curiosity.
