...
tag: The elegiac final BBC series — the original four-person lineup, one last run.
...
tag: The original — the touring series the format had not yet figured out it could fix in one tent.
slot_argument: Format-historical weight earns a high slot. The pilot moved from location to location across the UK rather than settling at Welford, and the editorial register was lighter than the show would later carry.
...
## 8. The Channel 4 Reset

tag: The format reboot — new network, new hosts, new judge, same tent.
slot_argument: A coherent reset that asked whether Bake Off could survive a wholesale change of presenters and judges in one move. The tent stayed at Welford, the format stayed intact, and the casting class did the work the change required.
...
tag: The series that confirmed the Welford peak — same tent, same lineup, sharpened format.
slot_argument: The format running at full confidence one year on from the canon's anchor series. The casting class lacks a single defining personality but the bench is dense end to end, and the technical challenges sharpen across the run.
...
## 4. The Format Settles

tag: The series the format clicked into place — Harptree Court, ten episodes, the rhythm Bake Off would carry forward.
slot_argument: The first series at the ten-episode length, with the technical bench fully formed and the casting class running deeper than the prior runs. The tent had not yet moved to Welford, but the show had figured out what it was.
...
## 9. The Channel 4 Settles

tag: The series the new lineup stopped being new — Toksvig, Fielding, Leith, Hollywood, running smoothly.
slot_argument: A confident sophomore run for the Channel 4 lineup. The casting class is dense, the bench is sharper than the prior year, and the tent has stopped self-consciously announcing the move.
...
## 3. The Move To Harptree

tag: The series the tent found its home — Bake Off moves to Harptree Court and the show starts looking like itself.
slot_argument: The first series that visually reads as Bake Off proper. The touring pilot is behind it, the tent is fixed to one estate, and the format pulls the casting and the bench into the rhythm fans now recognize.
...
## 10. Sandi's Final Year

tag: The series the Channel 4 era's first lineup signed off — Toksvig's last run at the tent flap.
slot_argument: A clean closing chapter for the Toksvig-Fielding presenter pairing. The casting class is strong, the tent reads settled, and the run carries the texture of a quiet era end.
