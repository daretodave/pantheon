---
show: dragrace
number: 13
title: Season 13
slug: season-13
premiere_date: 2021-01-01
location: Los Angeles, California
host: RuPaul Charles
format_changes: [opening-lip-sync-structure]
canonical_position: 14
eyebrow: "Aired winter 2021 · VH1 · the restructured premiere"
lede: "The season that opened with an unusual premiere lip-sync structure, reworking how the first episode functions. A large VH1 cast filmed under tightened production conditions."
premiere_caption: "VH1 · Friday 8/7c"
format_summary: "Standard format · new premiere"
format_caption: "restructured opening"
host_caption: "RuPaul, with Michelle Visage"
---

The season that rewired its first episode. Season 13 opens
with an unusual premiere lip-sync structure that changes the
opening hour and resets what episode one can be. RuPaul Charles
and Michelle Visage anchor the panel. The cast is large
and the production shows the marks of a tightened filming
environment, giving the season a contained feel. The premiere
experiment is the headline; the contest is solid but uneven.
Watch for the structural swing more than the craft.
