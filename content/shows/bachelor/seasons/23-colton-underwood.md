---
show: bachelor
number: 23
title: Colton Underwood
canonical_position: 13
premiere_date: 2019-01-07
ep_count: 12
location: Agoura Hills, California
host: Chris Harrison
eyebrow: "Aired winter 2019 · Former-NFL lead"
lede: "A former NFL practice-squad player takes the mansion after two prior runs across the sister franchises. The producers cast the season around a single biographical hook the franchise leaned into harder than any prior winter, and the marketing made it the whole pitch."
pull: "Underwood is the season the franchise built its entire promotional argument around one fact about the lead. The single-image pitch is the loudest the show had tried."
format_summary: "30 women · single-hook NFL lead"
format_caption: "the franchise's loudest single-image pitch"
filming_caption: "Agoura Hills · Singapore, Thailand, Portugal"
premiere_caption: "ABC · Monday 8/7c"
episodes_caption: "12 episodes including specials"
cast_size: 30
cast_size_caption: "30 women in the opening field"
host_caption: "Chris Harrison's twenty-third season"
watch_list:
  - episode_label: "Ep 1 · the single-hook frame"
    body: "The opening hour stages the lead's biography as the season's entire marketing argument. The franchise commits to a single-image pitch harder than any prior winter."
  - episode_label: "Ep 4 · the deepened roster"
    body: "The casting field reads as the modern-era recruitment pipeline at full strength. The bench is polished and the limo line plays sharp."
  - episode_label: "Ep 8 · Thailand stretch"
    body: "Travel runs through Thailand and Singapore, doing cinematic work the franchise had refined over the decade. The pacing tightens."
  - episode_label: "Ep 11 · the Portugal close"
    body: "Final dates move through Portugal, a destination the franchise had not tried at this point. The season's structure breaks from the era's expected runway."
---

A former NFL practice-squad player takes the mansion after two prior sister-franchise runs. Twelve episodes, thirty women, travel through Singapore, Thailand, and Portugal. The producers cast around a single biographical hook leaned into harder than any prior winter, and the marketing made it the whole pitch. Chris Harrison hosts a season remembered for how loud its single-image casting ran and for a finale runway that broke from the era's expected structure. A run defined by framing more than travelogue.
