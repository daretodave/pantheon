---
show: bachelor
number: 17
title: Sean Lowe
canonical_position: 6
premiere_date: 2013-01-07
ep_count: 12
location: Agoura Hills, California
host: Chris Harrison
eyebrow: "Aired winter 2013 · Texan lead"
lede: "The franchise's clean-slate season — a Dallas insurance agent and former college football player takes the mansion after his own run as a final-rose contender the prior year. The producers stage the season as a reset after a friction-heavy run, and the casting reads accordingly."
pull: "Lowe is the season the franchise decided to behave for a year. The result is one of the most-watched runs of the decade, and a quiet argument that the format still works at its most earnest."
format_summary: "25 women · Texas returnee lead"
format_caption: "casting reset after a friction-heavy run"
filming_caption: "Agoura Hills · St. Croix, Canada, Thailand"
premiere_caption: "ABC · Monday 8/7c"
episodes_caption: "12 episodes including specials"
cast_size: 26
cast_size_caption: "26 women in the opening field"
host_caption: "Chris Harrison's seventeenth season"
watch_list:
  - episode_label: "Ep 1 · the Texas frame"
    body: "The casting reveal positions the lead's regional identity hard — family, faith, hometown. The franchise's first deliberate move toward an unironic-earnest register."
  - episode_label: "Ep 5 · Canada stretch"
    body: "Travel opens with a Canada leg that does landscape work the prior winter had skipped. The pacing settles."
  - episode_label: "Ep 8 · St. Croix runway"
    body: "The Caribbean fantasy-suite stretch is shot patiently, and the season's argument tightens. The producers trust the cast to carry the beats."
  - episode_label: "Ep 11 · Thailand finale"
    body: "Final dates run through Thailand. The cinematography earns the runway, and the season closes on the format at its most relaxed."
---

A Dallas insurance agent and former college football player takes the mansion, returning to the franchise after a run the year prior. The producers cast the season as a reset — twelve episodes, a roster more measured than theatrical, travel through St. Croix and Thailand. The casting reads earnest in a way the franchise had been avoiding, and the result is one of the decade's most-watched runs. Chris Harrison hosts a season the franchise rebuilt its register around.
