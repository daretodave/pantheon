...
- [ ] Phase 31 — Canon + Community unification, rank wiring, season slugs, generation paths. **Headline rule: canon rank is ALWAYS working** — no season ships without `canonical_position`, no show with one or more seeded seasons ships without a real `canon.md`, every batch tick re-ranks and rebases surrounding entries. Rebuilds `/shows/[show]/canon` + `/shows/[show]/community` to share the new unified shell from `design/tiered.tv · Survivor Canon.html` (tier S/A/B/C bands, methodology cells, era toolbar, hero+mid+compact+tail entry layouts, community live-strip + movers + weekly-question card + full ranking table). Fixes the show-home filter chips (drops "By era", makes Canon/Community functional with shift pills), reworks the season-detail vote default to "community top 10" wording, extends the canon schema (editor / last_revised / methodology / tier blurbs / weekly_question / era_bands; canon entries gain tag + slot_argument + community_rank_hint), backfills `canonical_position` across all 47 existing season files, and authors a real canon.md for each of the 10 shows that don't have one. **Season URL slugs:** route renames `[n]` → `[slug]`; canonical form becomes `/shows/<show>/season/<slug>` (e.g. `/shows/survivor/season/marquesas`); old `/season/<digits>` 308s to slug. Slugs derive from the existing `NN-<slug>.md` filename convention — no new authoring required for existing files. Locks the always-working rule + slug convention in `.claude/agents/content-curator.md` + `skills/ship-content.md` (Rule 1 ships a populated canon with the first season; Rule 2 inserts each new season into the canon in the same tick with rationale + rebase; both rules document the `NN-<slug>.md` filename convention). `scripts/content-check.ts` gains the runtime guardrail (canon/rank invariants + slug uniqueness) that gates `pnpm verify`. **Sits ahead of phase 26 in the queue** so phase 26's drain produces canon-conforming + slug-canonical content from the moment phase 31 lands. Detailed brief: `phase_31_canon_rank_unification.md`.
- [ ] Phase 26 — Season backfill, full drain (10/tick) + canon long-truck. Rewritten in 26a to (a) raise the per-tick budget to 10 seasons, (b) drain every show until aired-season coverage is complete, (c) graduate each show into canon-mode once coverage clears the floor, writing a full ranked canon with rationales. Sits behind the redesign phases so the cloud loop sees the new pages as it ships content. Detailed brief: `phase_26_season_backfill.md`.
