...
| Per-show facade SVG | generated by `brander` | re-rendered by `brander` if critique flags | Claude Code agent |
| Per-show palette | derived by `brander` from facade analysis | manual override possible | Claude Code agent |
...
│   └── shows/<slug>/{sigil,facade}.svg     # generated by brander
├── src/
...
The user emits design exports asynchronously. Format: claude-design
JSX + JSON. Authoritative reads:

- `design/tokens.json` — finalized design tokens (canonical;
  Tailwind + CSS vars derive from this).
- `design/compositions/sigils.jsx` — per-show sigil
  compositions; the brander agent extends this set.
- `design/compositions/screens.jsx` + `screens.css` — page
  composition reference; phase 6 (show home) and phase 9
  (season page) implement against these.
- `design/Pantheon · {Compositions,Facades,Tokens}.html` —
  visual previews for human review.
- `design/checks/*.jpg` — review iterations (read-only audit
  trail).
...
> "Pantheon now reads top-to-bottom as one system. Warm-tinted
> dark paper, a single ceremonial gold, six muted sentiment hues,
> and a strict single-1px shadow contract carry the brand. The
> facade grammar — column / pediment / frieze / ornament on a
> 1200 × 800 frame — gives every show a unique architectural
> surface from a shared vocabulary, with sigils derived (never
> redrawn) by cropping the pediment + center column to a 320 ×
> 320 square. Three real pages turn that grammar into product:
> a cold-search home hero that establishes the promise in three
> seconds, a show-home that takes over the chrome with its
> three-color palette, and a season page whose vote pair,
> comment input, and 72-hour rank pill are working components —
> not mocks. The promise (\"the seasons, ranked. no spoilers.\")
> is reinforced at every depth: pediment flame, hero headline,
> shield pill, vote question, inline comment reminder."
> — design hand-off, 2026-05-12

Locked specifics:
- **Mode:** dark default + light opt-in. Toggle lives in the
  footer (not the header — chrome stays clean). Persisted in
  `localStorage.pantheon_theme`. Ships in phase 1.
- **Palette:** `design/tokens.json` is canonical. Dark primary
  is the ceremonial gold (`#E8B65A`); light primary is the
  warm clay (`#8B5A1F`). Never override paper / ink / line
  colors per-show.
- **Per-show palette swap:** ONLY the show home, canon,
  community, and season pages take on the show's 3-color
  primary tint. The chrome (header / footer / nav) stays in
  Pantheon's ceremonial gold. The "magical" reveal moment is
  navigating *into* a show — the page chrome shifts hue while
  the site chrome stays constant. Implementation: CSS custom
  properties scoped to `[data-show=<slug>]` on the page wrapper.
- **Type families:** Source Serif 4 (headlines / blurbs),
  Inter (UI), JetBrains Mono (rank numbers, meta).
...
## Facade grammar (the per-show visual contract)

Every show in `content/shows/<slug>.md` carries (frontmatter
fields):
...
The `brander` sub-agent owns:
- `public/shows/<slug>/facade.svg` — full hero composition,
  1200 × 800 viewBox, four slots (column / pediment / frieze /
  ornament).
- `public/shows/<slug>/sigil.svg` — derived from facade by
  cropping pediment + center column to 320 × 320. Never drawn
  separately; always cropped from the facade source.
- `public/shows/<slug>/ornament-{1,2,3}.svg` — small
  decorative SVGs reused on season pages.

The grammar is shared across all shows; the motifs are unique
per show. Phase 4 (facade primitives) ships the SVG component
library; phase 5 (first 3 shows facades) ships Survivor + Top
Chef + Drag Race; subsequent shows are commissioned via
`/ship-content` (which spawns `brander` for facade work).
...
### Rule 4 — facade completeness
Every covered show must have `public/shows/<slug>/facade.svg`,
`sigil.svg`, and ≥ 1 ornament. If a show was added without
facade (escape hatch in Rule 1), file a row that dispatches to
`brander` directly (no `content-curator` needed).
